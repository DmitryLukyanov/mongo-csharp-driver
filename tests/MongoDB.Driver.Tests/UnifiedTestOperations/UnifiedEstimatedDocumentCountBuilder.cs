...
namespace MongoDB.Driver.Tests.UnifiedTestOperations
{
    public class UnifiedEstimatedDocumentCountOperation : IUnifiedEntityTestOperation
    {
        ...
        public OperationResult Execute(CancellationToken cancellationToken)
        {
            try
            {
                var result = _collection.EstimatedDocumentCount(_options, cancellationToken);
        ...
        public async Task<OperationResult> ExecuteAsync(CancellationToken cancellationToken)
        {
            try
            {
                var result = await _collection.EstimatedDocumentCountAsync(_options, cancellationToken);
    ...
    public class UnifiedEstimatedDocumentCountOperationBuilder
    {
        ...
        public UnifiedEstimatedDocumentCountOperation Build(string targetCollectionId, BsonDocument arguments)
        {
            ...
            foreach (var argument in arguments?.Elements ?? Enumerable.Empty<BsonElement>())
            {
                switch (argument.Name)
                {
                    case "maxTimeMS": estimatedDocumentCountOptions.MaxTime = TimeSpan.FromMilliseconds(argument.Value.AsInt32);
                        break;
                    default:
                        throw new FormatException($"Invalid {nameof(UnifiedEstimatedDocumentCountOperation)} argument name: '{argument.Name}'.");
                }
            }
            return new UnifiedEstimatedDocumentCountOperation(collection, options: estimatedDocumentCountOptions);
        }
    }
}
