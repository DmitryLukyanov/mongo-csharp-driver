...
namespace MongoDB.Driver.Core.Configuration
{
    ...
    public sealed class ConnectionString
    {
        ...
        private void Parse()
        {
            ...
            {
                // todo: message
                throw new MongoConfigurationException("DirectConnection=true is not allowed to be used with SRV.");
            }

            if (_hosts.Count > 1 && _connect == ClusterConnectionMode.Direct)
            {
                // todo: message
                throw new MongoConfigurationException("DirectConnection=true is not allowed to be used with multiple seeds.");
            }
