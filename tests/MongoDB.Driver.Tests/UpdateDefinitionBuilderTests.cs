...
namespace MongoDB.Driver.Tests
{
    public class UpdateDefinitionBuilderTests
    {
        ...
        {
            var subject = CreateSubject<Person>();
            string expectedErrorMessage = "Array indexes must be more or equal than -1.";

#pragma warning disable 251
            AssertThrow<Person, NotSupportedException>(subject.Set(x => x.FavoriteColors[-2], "yellow"), expectedErrorMessage);
#pragma warning restore
            AssertThrow<Person, NotSupportedException>(subject.Set(x => x.Pets[-2].Name, "Fluffencutters"), expectedErrorMessage);
            AssertThrow<Person, NotSupportedException>(subject.Set(x => x.Pets.ElementAt(-2).Name, "Fluffencutters"), expectedErrorMessage);
        }
